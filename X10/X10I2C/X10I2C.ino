...
extern byte HouseCode = 'A' ;
extern byte UnitCode = 0 ;
extern byte Commande = 0;
extern int flag = 0;
...
void setup(){
  pinMode(X10, OUTPUT); //port pour le module AUREL
  pinMode(LED, OUTPUT); //visualisation LED
...
void loop(){
  if(flag==1)
  {
    x10_sendcmd(HouseCode, UnitCode, Commande);
    Serial.println("commande envoyée");
    Serial.println(HouseCode);
    Serial.println(UnitCode);
    Serial.println(Commande);
    Serial.println("fin commande");
    flag = 0;
  }
  delay(100);
}
